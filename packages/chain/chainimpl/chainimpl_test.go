...
func TestMempoolClean(t *testing.T) {
	env, _ := chainimpl.NewMockedEnv(t, true)

	/*requests := requestsOnLedgerArrayToRequestsArray(env.GetRequestsOnLedger(t, 5))
	env.Log.Infof("XXX REqUESTS %v", len(requests))
	env.NextState(requests...)*/
	tx, targetAddr := env.GetTxWithRequestsOnLedger(t, 5)
	env.NextState(tx, targetAddr)

	time.Sleep(3 * time.Second)
}

func requestsOnLedgerArrayToRequestsArray(in []*request.RequestOnLedger) []coretypes.Request {
	out := make([]coretypes.Request, len(in))
	for i := range in {
		out[i] = in[i]
	}
	return out
